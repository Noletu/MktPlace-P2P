...
echo "📋 1.5 - Logout - João"
curl -s -X POST "$API_URL/auth/logout" -b cookies_joao.txt > /dev/null
test_passed "João deslogado"
echo ""
...
RESPONSE=$(curl -s -X POST "$API_URL/auth/register" \
  ...
  -c cookies_maria.txt \
  -d '{
    "email": "maria.compradora@teste.com",
    "cpf": "52998224725",
...
if echo "$RESPONSE" | grep -q '"success":true'; then
  TRANSACTION1_ID=$(echo "$RESPONSE" | grep -o '"transactionId":"[^"]*' | head -1 | sed 's/"transactionId":"//')
...
# Imagem fake base64 (1x1 pixel PNG transparente)
...
RESPONSE=$(curl -s -X POST "$API_URL/auth/register" \
  ...
  -c cookies_ana.txt \
  -d '{
    "email": "ana.investidora@teste.com",
    "cpf": "84434408045",
...
if echo "$RESPONSE" | grep -q '"success":true'; then
  TRANSACTION2_ID=$(echo "$RESPONSE" | grep -o '"transactionId":"[^"]*' | head -1 | sed 's/"transactionId":"//')
...
RESPONSE=$(curl -s -X POST "$API_URL/auth/register" \
  ...
  -d '{
    "email": "carlos.diverso@teste.com",
    "cpf": "23767912084",
...
echo "📋 6.1 - Verificar Marketplace (deve estar vazio ou com novos pedidos)"
...
echo "📋 6.2 - Verificar Meus Pedidos - João (deve ter 1 pedido VALIDATING)"
RESPONSE=$(curl -s -X GET "$API_URL/orders/my-orders" -b cookies_joao.txt)
if echo "$RESPONSE" | grep -q "$ORDER1_ID"; then
  test_passed "João vê seu pedido R$2.000 (status: VALIDATING)"
else
  test_failed "João não vê seu pedido" "$RESPONSE"
fi
echo ""

echo "📋 6.3 - Verificar Meus Pedidos - Pedro (deve ter 1 pedido VALIDATING)"
RESPONSE=$(curl -s -X GET "$API_URL/orders/my-orders" -b cookies_pedro.txt)
if echo "$RESPONSE" | grep -q "$ORDER2_ID"; then
  test_passed "Pedro vê seu pedido R$500 (status: VALIDATING)"
else
  test_failed "Pedro não vê seu pedido" "$RESPONSE"
fi
echo ""

echo "📋 6.4 - Verificar Meus Pedidos - Carlos (deve ter 1 pedido CANCELLED)"
RESPONSE=$(curl -s -X GET "$API_URL/orders/my-orders" -b cookies_carlos.txt)
if echo "$RESPONSE" | grep -q "$ORDER3_ID"; then
  test_passed "Carlos vê seu pedido R$1.200 (status: CANCELLED)"
else
  test_failed "Carlos não vê seu pedido" "$RESPONSE"
fi
echo ""
...
echo "  1. João Vendedor    - CPF: 11144477735 (Vendeu BTC por R$2.000)"
echo "  2. Maria Compradora - CPF: 52998224725 (Comprou BTC pagando R$2.000)"
echo "  3. Pedro Trader     - CPF: 39053344705 (Vendeu USDT por R$500)"
echo "  4. Ana Investidora  - CPF: 84434408045 (Comprou USDT pagando R$500)"
echo "  5. Carlos Diverso   - CPF: 23767912084 (Criou e cancelou pedido ETH)"
echo ""
