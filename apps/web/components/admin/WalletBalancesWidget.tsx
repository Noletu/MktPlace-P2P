...
export default function WalletBalancesWidget() {
  ...
  if (loading) return <div className="text-center py-4 text-gray-300">Carregando...</div>;
  ...
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-lg shadow-xl p-6">
      ...
      {wallets.length === 0 ? (
        <p className="text-sm text-gray-400">Nenhuma carteira configurada</p>
      ) : (
        <div className="space-y-3">
          {wallets.map((wallet) => (
            <div key={wallet.id} className="flex items-center justify-between border-b border-gray-700 pb-3">
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className="font-semibold text-white">{wallet.cryptoType}</span>
              ...
              <div className="text-right">
                <p className="text-sm font-bold text-white">{wallet.balance || '0.00'}</p>
                <p className="text-xs text-gray-400">$ {wallet.valueUSD || '0.00'}</p>
              </div>
            </div>
          ))}
        </div>
