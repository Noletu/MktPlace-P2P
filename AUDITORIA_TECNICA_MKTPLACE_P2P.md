...
>
> **Changelog v1.41:** MED-33 (Fatia 1) — 2º model do `metadata` migrado: `PlatformWalletMovement` (`String?`→`Json?`/`jsonb`, Sprint 3 sessão 11). Write-only, único ponto `platformWallet.service.ts:327` (`recordMovement`): `JSON.stringify`→objeto + `Prisma.DbNull`; zero reads-parse. Cobertura por **paridade** com `Notification` (v1.40): coluna `jsonb` confirmada empiricamente + `tsc` 23 — sem smoke e2e dedicado (exigiria criar um `PlatformWallet` de teste com FK; critério análogo ao `disputeData`). metadata 2/5 models; restam `WalletTransaction`, `AuditLog`, `AdminAction`.
>
...
>
> **Changelog v1.25:** SER-39 fechado (Sprint 3 sessão 8): **defesa contra brute-force de código 2FA** no `/auth/complete-login`, implementada **paralela** ao SER-22 (vetores ortogonais — campos separados, contadores separados, lockouts separados). 4 campos novos no `User` (`failed2FAAttempts`, `twoFactorLockedUntil`, `twoFactorLockoutCount`, `last2FAFailAt`); **service dedicado** `twoFactorLockoutService` (espelha estruturalmente o `accountLockoutService`, duplicação intencional para isolamento/zero risco no código de senha em produção). **Threshold 5** (mais permissivo que senha=3, por UX de TOTP/backup codes); **backoff exponencial** 5min→30min→2h→24h (cap 4); cooldown 30min, count-reset 24h. **Enforcement no Caso C** do `completeLogin`: `isLockedFor2FA` checa **antes** de `verifyToken` (Caminho Z — não roda verificação de código se a conta está em lockout), `recordFailed2FA` no ramo de código errado (complementar ao `attemptsRemaining` do `PendingLogin`: este limita brute-force **dentro** de uma sessão, SER-39 limita **entre** sessões), `recordSuccessful2FA` reseta no ramo de sucesso. Efeitos colaterais: audit `TWO_FACTOR_LOGIN_LOCKED` (com `ipAddress`, disponível no controller — incremento sobre o `ACCOUNT_LOGIN_LOCKED`, que rodava no service sem `req`) + e-mail **tom grave** (paleta vermelha `#dc2626` consistente com `sendEmergencyOverrideEmail`; texto destaca "sua senha foi inserida corretamente — pode estar comprometida"; CTA "Trocar senha agora" → `/forgot-password`; enviado sempre). **`twoFactorLimiter` adicionado** ao `/complete-login` (defesa em profundidade por IP). **Guard de idempotência aplicado desde o início** (não foi ajuste posterior como no SER-22) e **validado empiricamente** sob concorrência real (HTTP-2FA-12: 5 requests paralelas escalam `lockoutCount` apenas +1, geram apenas 1 audit/email). **Correção da nota da Sessão 7:** o `twoFactorLimiter` **já estava montado** em `twoFactor.routes.ts` (`/enable`, `/disable`, `/regenerate-backup-codes`) — só não estava no `/complete-login`, gap que esta sessão fecha. Validação manual 12/12 (HTTP-2FA-1..12) + 2 e-mails Ethereal conferidos (paleta vermelha distinta do âmbar do SER-22). Baseline inalterado (tsc 25, jest 59/18/40). §1.1 PRE-STAGING: 24 − 1 (SER-39) = **23** itens.
>
...
| MED-33 | Migrar para `Json`/`jsonb` (junto com CRIT-01) | 🔶 **Parcial** — 4 campos + `metadata` 3/5 models: `disputeData` (v1.36), `notificationPreferences` (v1.37), `twoFactorBackupCodes` (v1.38), `orderData` (v1.39); `metadata` de `Notification` (v1.40), `PlatformWalletMovement` (v1.41) e `AdminAction` (v1.42). Pendente: 2 models do `metadata` (`WalletTransaction`, `AuditLog`) + validação Zod ([PRE-STAGING]). |
...
> **Nota de v1.25 (Sprint 3 sessão 8):** o guard de idempotência atômico introduzido aqui foi **confirmado empiricamente sob concorrência real** no SER-39, que reusa o mesmo padrão (`increment` atômico + trigger só no `=== THRESHOLD` exato). O teste HTTP-2FA-12 disparou 5 requests paralelas com o mesmo token e verificou que o contador parou em exatamente 5 (não >5), o backoff escalou apenas +1 e apenas 1 audit/e-mail foi gerado — validação que a Sessão 7 (SER-22) não chegou a executar.
...
**Severidade:** 🟡 Médio
**Fase:** 🟡 **[EM PROGRESSO — PARCIAL]** — `disputeData` (v1.36), `notificationPreferences` (v1.37), `twoFactorBackupCodes` (v1.38) e `orderData` (v1.39) migrados. O 5º campo `metadata` está em 5 models: `Notification` (v1.40), `PlatformWalletMovement` (v1.41) e `AdminAction` (v1.42) feitos; restam `WalletTransaction`, `AuditLog`; validação Zod robusta → 🔵 **[PRE-STAGING]**
**Esforço estimado:** 1 dia (após CRIT-01)
**Status:** 🟡 Em progresso — 4 campos migrados (`disputeData` v1.36, `notificationPreferences` v1.37, `twoFactorBackupCodes` v1.38, `orderData` v1.39); o 5º (`metadata`) abrange 5 models — `Notification` (v1.40), `PlatformWalletMovement` (v1.41) e `AdminAction` (v1.42) migrados, restam `WalletTransaction`, `AuditLog`
...
- [ ] Campo `metadata` migrado (5 models — `Notification` ✅ v1.40, `PlatformWalletMovement` ✅ v1.41, `AdminAction` ✅ v1.42; restam `WalletTransaction`, `AuditLog`)
...
**v1.37 — `notificationPreferences`** (model `User`, nullable):
- Toda a (de)serialização estava encapsulada em `notificationPreferences.service.ts` (1 `JSON.parse`, 1 `JSON.stringify`); os 3 consumers só repassam o valor. Código contido a 1 arquivo.
...
**v1.39 — `orderData`** (model `Order`, **NOT NULL**, campo central — 6 arquivos):
- Migration com fallback: `ALTER COLUMN ... USING (CASE WHEN ... IS NULL OR = '' THEN '{}'::jsonb ELSE ...::jsonb END)` — `{}` cobre bordas sem violar o NOT NULL (que o `ALTER TYPE` preserva).
...
- Restam (após v1.40): `WalletTransaction`, `PlatformWalletMovement`, `AuditLog`, `AdminAction`.

**v1.41 — `metadata` do model `PlatformWalletMovement`** (2 de 5 models):
- Write-only, único ponto `platformWallet.service.ts:327` (`recordMovement`): `JSON.stringify`→objeto + `null`→`Prisma.DbNull`; zero reads-parse; interface já `Record<string,any>`. Idêntico ao `Notification`.
- Cobertura por **paridade** (sem smoke e2e dedicado): coluna `jsonb` confirmada empiricamente + `tsc` 23 + código idêntico ao `Notification` (validado 4/4). Smoke real exigiria criar um `PlatformWallet` de teste (FK) — critério análogo ao `disputeData`.
- Restam 3 models: `WalletTransaction`, `AuditLog`, `AdminAction`.

**v1.42 — `metadata` do model `AdminAction`** (3 de 5 models):
- Diferente dos anteriores: o gravador central `logAdminAction` (`admin.service.ts`) faz passthrough; os **9 callers** é que fazem `JSON.stringify`. Write-only (zero `JSON.parse` de leitura).
- **Opção A** (mínima): `logAdminAction` passou a aceitar `string | Record<string,any>` e normaliza antes de gravar — `typeof === 'string' ? JSON.parse : objeto`, `Prisma.DbNull` no vazio. 1 edição; os 9 callers ficam intactos (passam string; o service converte pro `jsonb`).
- **Dívida menor (não-bloqueante):** limpar os 9 `JSON.stringify` dos callers de `logAdminAction` (passar objeto direto) — a tolerância do service cobre a transição.
- Validação: `tests/e2e/med33-adminaction-metadata-check.ts` (6/6) — coluna `jsonb`, caminho string (legado)→object, caminho objeto (futuro)→object, valores preservados, sem-metadata→NULL.
- Restam 2 models: `WalletTransaction`, `AuditLog`.
...
### Critério de aceitação
- [x] Socket CORS usa configuração consistente com o HTTP (flag explícita ou whitelist compartilhada)

### ✅ Resolução (Sprint 3 sessão 11 — v1.30)

Whitelist de origins extraída para fonte única `getAllowedOrigins()` em `apps/api/src/config/cors.ts` (lazy: lê `process.env.ALLOWED_ORIGINS` na chamada, não no import, para não depender da ordem de carga do dotenv). O CORS HTTP (`index.ts`) e o do Socket.IO (`socket.server.ts`) consomem a mesma função — fim da divergência. O ternário `NODE_ENV` foi removido do socket, que agora é **fail-secure** (origin fora da whitelist é rejeitada em qualquer ambiente), consistente com o HTTP.

**Validação:** por inspeção + `tsc` (a mesma `getAllowedOrigins()` é exercida de ponta a ponta no caminho HTTP pelo e2e do SER-32). Não foi montado e2e de handshake de Socket.IO — a mudança é troca da fonte da whitelist, sem nova lógica.

**Commits:** branch `fix/ser31-32-cors-consistency`, F1 `ee5ab6d`.
...
### Critério de aceitação
- [x] Rejeição CORS não gera mais 5xx (ou retorna 403 explícito)
- [x] Logs de rejeição não aparecem como "Unhandled error"

### ✅ Resolução (Sprint 3 sessão 11 — v1.30)

A rejeição de origin deixou de propagar como 500. No callback do CORS (`index.ts`), em vez de um `Error` cru, joga-se um erro com `statusCode = 403`. O error handler global passou a respeitar `err.statusCode` (`res.status(err.statusCode || 500)`) e a só logar "Unhandled error" para 5xx — rejeições 4xx (esperadas) não poluem mais logs nem inflam métricas de erro de servidor (a `origin` function já registra cada rejeição como `warn`).

**Postura preservada:** optou-se por **manter o bloqueio no servidor** e apenas corrigir o status para 403, em vez do `callback(null, false)` sugerido originalmente (que deixaria o servidor *processar* a request de origin não-autorizada, delegando a proteção ao preflight do browser) — escolha conservadora para uma plataforma custodial.

**Validação:** e2e `apps/api/tests/e2e/ser32-cors-status.ts` (HTTP real, server de pé) — origin não-autorizada → 403, sem origin (flag false) → 403, origin autorizada → passa o CORS. 3/3.

**Commits:** branch `fix/ser31-32-cors-consistency`, F2 `8d5cb01`, e2e `11bd515`.
...
**Fase:** ✅ **Resolvido (v1.26)**
**Categoria:** Auth / Cripto
**Status:** ⬜ Catalogado (Sprint 3 sessão 6) · 🔬 Refinado empiricamente (Sprint 3 sessão 7) · ✅ **Fechado (v1.26, Sprint 3 sessão 9)**
...
### Resolução (v1.26, Sprint 3 sessão 9)
Implementado na branch `fix/ser38-bcrypt-cost`:
- **`utils/bcrypt.ts`:** `SALT_ROUNDS` 10 → 12 e exportado; novo helper `needsRehash(hash)` (`bcrypt.getRounds(hash) < SALT_ROUNDS`).
- **`auth.service.ts` (`verifyCredentials`):** rehash transparente best-effort — só **depois** de a senha ser confirmada correta (não introduz leak novo); se `needsRehash` for true, regrava o hash em cost-12. Falha de rehash é logada e engolida (`// NÃO re-throw`), para nunca quebrar um login válido.
- **`prisma/seed.ts`:** unificado para usar o `hashPassword()` central em vez de `bcrypt.hash` direto (elimina a divergência de cost na fonte).
- **Backup codes 2FA:** passam a cost-12 automaticamente (mesmo `hashPassword`).

**Validação:** 4 testes unitários (`utils/__tests__/bcrypt.spec.ts`) + 2 de integração (`services/__tests__/auth.ser38-rehash.spec.ts`, contra Postgres real) confirmam: senha cost-10 é regravada para cost-12 no primeiro login bem-sucedido; senha já cost-12 fica intacta. Baseline preservado (unit 59 → 63 passed; a integração roda isolada com `DATABASE_URL` setado e dá skip no `npm test` padrão).

**Critério 3 (gap < 15%) — resultado honesto:** o componente dominante do leak medido na sessão 7 (gap de ~3.6× / ~265%, causado pela diferença de cost entre o dummy cost-12 e contas cost-10) foi **eliminado** — ambos os caminhos agora computam cost-12. Porém, medição empírica pós-fix revelou um **resíduo de ~95ms** que **não** vem do bcrypt, e sim da assimetria de I/O: o caminho de conta real faz `isLocked` (1 read) + `recordFailedLogin` (2 updateMany + 1 update que escreve) que o caminho "email inexistente" não faz (~447ms sem conta vs ~538ms conta real = 16,8%). Uma tentativa de equalização por "dummy I/O" foi prototipada e **descartada nesta sessão**: igualava o número de round-trips mas não o custo da escrita real (update com fsync), então o resíduo absoluto permanecia ~95ms — o % só caía por inflar o denominador. Decisão: manter o fix sólido do bcrypt, reverter os dummies frágeis, e registrar o resíduo de I/O como **SER-40** (severidade baixa). O vetor de maior valor (salto de timing pós-lockout do SER-22) fica fechado de qualquer forma, pois dummy e contas passam a ter o mesmo custo de hash.

### Critério de aceitação
- [x] `SALT_ROUNDS=12` ✅ (v1.26)
- [x] Rehash transparente de senhas cost-10 no login ✅ (v1.26, best-effort após senha correta)
- [~] Gap de timing entre os dois caminhos < 15% para qualquer conta — **parcial**: o componente de custo do bcrypt (~3.6×) foi eliminado; o resíduo de I/O (~95ms) permanece e foi spin-off para o **SER-40**
...
**Status:** ✅ **Fechado (v1.25, Sprint 3 sessão 8).** Lockout automático por conta para falhas de **código 2FA**, **paralelo e ortogonal** ao SER-22 (campos, contadores e lockouts separados). Validado por HTTP (12/12, HTTP-2FA-1..12, incluindo teste de concorrência) + 2 e-mails Ethereal conferidos visualmente.
...
Lockout dedicado a 2FA, **paralelo** ao SER-22 (decisão Sessão 8: aceitar ~80% de duplicação estrutural em troca de isolamento e **zero risco** no código de senha já em produção).

#### Schema (`User`, migration `20260607213406_ser39_2fa_lockout`)
```prisma
failed2FAAttempts     Int       @default(0)
twoFactorLockedUntil  DateTime?
twoFactorLockoutCount Int       @default(0)   // define o backoff
last2FAFailAt         DateTime?
```

#### `twoFactorLockoutService` (novo, `services/twoFactorLockout.service.ts`)
- Espelha estruturalmente o `accountLockoutService`: `isLockedFor2FA` (pure read), `recordSuccessful2FA` (zera `failed2FAAttempts`+`twoFactorLockedUntil`, preserva `twoFactorLockoutCount`), `recordFailed2FA` (4 passos atômicos: reset 24h → reset 30min/lockout-expirado → `increment` atômico → trigger só no `=== THRESHOLD` exato).
- **Tipos locais** (`TwoFALockStatus`/`Failed2FAResult`) — zero acoplamento com o service de senha.
- **Guard de idempotência aplicado desde o início** (`> THRESHOLD` retorna sem re-escalar) — diferente do SER-22, onde foi ajuste posterior.

#### Parâmetros
- **Threshold:** 5 falhas consecutivas (mais permissivo que senha=3 — UX de TOTP/backup codes).
- **Backoff exponencial** por `twoFactorLockoutCount` (cap em 4): 5min → 30min → 2h → 24h.
- **Resets:** `failed2FAAttempts` esfria após 30min (ou 2FA OK); `twoFactorLockoutCount` esfria após 24h.

#### Integração (`completeLogin`, Caso C — usuário COM 2FA, código fornecido)
- **Caminho Z:** `isLockedFor2FA` checa **antes** de `verifyToken` — conta em lockout recebe **401 uniforme** ("Tentativas esgotadas. Faça login novamente.", mesma do esgotamento do `PendingLogin`) e **não** roda verificação de código nem incrementa contador.
- Ramo `!totpValid` → `recordFailed2FA` (**complementar** ao `attemptsRemaining`: o `PendingLogin` limita brute-force **dentro** de uma sessão, SER-39 **entre** sessões). Disparo → `notifyTwoFactorLockout`: audit `TWO_FACTOR_LOGIN_LOCKED` (com `ipAddress` — disponível no controller) + e-mail **tom grave** (paleta vermelha `#dc2626`, "senha pode estar comprometida", CTA "Trocar senha agora"), best-effort.
- Ramo de sucesso → `recordSuccessful2FA` reseta os contadores.
- **`twoFactorLimiter`** adicionado ao `/complete-login` (defesa em profundidade por IP). *Correção da nota da Sessão 7:* o limiter **já estava montado** em `twoFactor.routes.ts` (`/enable`, `/disable`, `/regenerate-backup-codes`) — só faltava no `/complete-login`.

### Critério de aceitação
- [x] Falhas de 2FA acumulam num contador durável por conta (não só por `PendingLogin`) — HTTP-2FA-4/5
- [x] Recriar o `PendingLogin` não zera o contador de falhas de 2FA — HTTP-2FA-5 (cada falha em sessão nova ainda incrementa `failed2FAAttempts`)
- [x] Threshold de 2FA dispara lockout (próprio, threshold=5, backoff exponencial) — HTTP-2FA-6 (5min), HTTP-2FA-9 (30min), audit confirma 5/30/120min
- [x] Caminho Z: conta em lockout barra **antes** de `verifyToken`, sem incrementar — HTTP-2FA-7 (código correto + conta locked → 401, `failed2FAAttempts` inalterado)
- [x] Guard de idempotência sob concorrência: 5 requests paralelas escalam `lockoutCount` apenas +1 e geram 1 audit/email — HTTP-2FA-12
...
### Problema
Depois que o SER-38 igualou o custo do bcrypt (ambos os caminhos do `verifyCredentials` agora cost-12), sobra um gap de timing menor que **não** vem do hash, e sim da assimetria de I/O entre os caminhos. O caminho de conta real faz trabalho de banco que o caminho "email inexistente" não faz: `accountLockoutService.isLocked` (1 read) + `recordFailedLogin` (2× `updateMany` + 1× `update` — este **escreve** uma linha). O caminho sem conta retorna mais cedo, sem esse I/O. Resultado: respostas de "senha errada em conta real" ficam ~95ms mais lentas que "email inexistente", permitindo a um atacante distinguir conta existente de inexistente pelo tempo de resposta (account enumeration).

### Confirmação empírica (Sprint 3 sessão 9)
Medido logo após o fix do SER-38: email inexistente **~447ms** vs conta real / senha errada **~538ms** (gap ~91ms / **16,8%**). O gap é dominado por uma escrita real (`update` com fsync no Postgres), não por leitura — por isso espelhar só as leituras não o fecha.

### Por que severidade baixa
O valor absoluto (~95ms) está dentro do ruído de latência de rede de qualquer ambiente que não seja localhost; o oráculo é mensurável em bancada mas frágil sob rede real. Além disso, o vetor de maior valor do mesmo leak (o salto de timing pós-lockout do SER-22, ~3.6×) já foi fechado pelo SER-38.

### Recomendação
Equalizar o I/O entre os caminhos de modo que iguale o **custo real** (não só o número de round-trips), em sessão dedicada. Duas abordagens:
- **(a) Escrita sentinela:** o caminho sem conta executa um `update` equivalente contra uma linha-sentinela dedicada, para que ambos os caminhos paguem o custo de uma escrita com fsync. Mais preciso, porém acopla ao schema.
- **(b) Piso de tempo constante:** medir o tempo decorrido e dormir até um teto fixo (ex.: 600ms) em todos os caminhos de falha de login, tornando a resposta independente do trabalho real. Mais simples e robusto, porém adiciona latência fixa.
...
| **TD-SCHEMA01** | Sequence `user_hd_account_seq` criada via raw SQL — Prisma gerava `DROP SEQUENCE` em toda migration que tocasse o model `User`. **Causa raiz:** migration `20260520000000_add_hd_account_index` usou `CREATE SEQUENCE` diretamente em SQL bruto (não via DDL do Prisma). O schema.prisma declarava `@default(dbgenerated("nextval('user_hd_account_seq'::regclass)"))`, que o diff engine do Prisma tratava como sequência externa desconhecida — gerando `DROP SEQUENCE` a cada nova migration no User. **Status:** ✅ **Resolvido (Sprint 3 sessão 3 — v1.20).** O sintoma observado (DROP SEQUENCE recorrente em migrations futuras do User) foi eliminado pela mudança de `@default(dbgenerated("nextval(...)"))` para `@default(autoincrement())` no schema.prisma. Verificação empírica confirmou: o diff engine do Prisma normaliza ambas as representações (sequence raw SQL no DB, autoincrement no schema) para o mesmo conceito interno, e portanto não gera mudança no SQL ao tocar o User table. **O que NÃO foi feito (intencionalmente):** a migração estrutural para `GENERATED BY DEFAULT AS IDENTITY` (Opção A na análise da Fase 3) foi descartada por análise de risco vs benefício. Razões: (a) o sintoma já estava resolvido pela mudança de schema apenas; (b) migration manual em coluna que sustenta o CRIT-02 carrega risco operacional não-zero (START WITH preciso, sequence interna auto-nomeada, possibilidade de colisão de índices); (c) sequence raw SQL é padrão Postgres válido — só era problemática pelo comportamento do diff engine do Prisma, que agora está normalizado. **Estado do banco pós-Sessão 3:** a sequence `user_hd_account_seq` permanece como objeto Postgres funcional, alimentando o DEFAULT da coluna `hdAccountIndex`. Continua nomeada descritivamente. É opaca ao Prisma, que a vê como `autoincrement()`. **Rollback plan preservado:** `docs/rollback-plans/td-schema01.md` documenta o procedimento completo de migração para IDENTITY, incluindo SQL transacional, cálculo de START WITH, e gatilhos de rollback — caso uma necessidade futura justifique a migração estrutural. **Verificação empírica do drift eliminado (Fase 3, Sessão 3):** Teste 1: `migrate diff` schema.prisma vs DB atual → sem diferença (esperado e correto). Teste 2: campo fake adicionado em Notification, `migrate diff` rodado → SQL gerado contém APENAS `ADD COLUMN` do campo fake, sem nenhuma mexida em User, `hdAccountIndex`, `user_hd_account_seq`, DROP DEFAULT ou DROP SEQUENCE. **CRIT-02 testado end-to-end:** novo usuário criado em banco dev recebeu `hdAccountIndex=33` (próximo valor correto da sequence, `last_value=32`, `is_called=t`); derivation paths BIP32 distintos para índices 3, 4 e 33 confirmados. **Identificado em:** Sprint 3 sessão 2. **Resolvido em:** Sprint 3 sessão 3 — branch `fix/td-schema01-hd-account-identity`. | ✅ **Resolvido** — removido do bucket bloqueante. O finding permanece catalogado; sua história é parte do conhecimento do projeto. | `docs/rollback-plans/td-schema01.md` preserva o plano de migração para IDENTITY caso seja necessário no futuro. **Relacionado a:** CRIT-02. | — |
...
Última edição: 07/06/2026 (v1.25 — Sprint 3 sessão 8: SER-39 ✅ fechado — lockout por brute-force de código 2FA, paralelo e ortogonal ao SER-22; 4 campos novos no User + twoFactorLockoutService dedicado (threshold 5, backoff 5min→30min→2h→24h); enforcement no Caso C do completeLogin com Caminho Z antes de verifyToken; audit TWO_FACTOR_LOGIN_LOCKED com ipAddress + e-mail tom grave (paleta vermelha, "senha pode estar comprometida"); twoFactorLimiter adicionado ao /complete-login; guard de idempotência validado empiricamente sob concorrência (HTTP-2FA-12); correção da nota da Sessão 7 sobre twoFactorLimiter já estar montado nas rotas de gerenciamento de 2FA; validado HTTP 12/12 + 2 e-mails Ethereal; baseline tsc 25/jest 59-18-40; §1.1 PRE-STAGING: 24 − 1 = 23 itens)
Edição anterior: 05/06/2026 (v1.24 — Sprint 3 sessão 7: SER-22 ✅ fechado — account lockout automático por conta com backoff exponencial 5min→30min→2h→24h, service dedicado com atomicidade anti lost-update, Caminho Z preservando timing do SER-23, audit ACCOUNT_LOGIN_LOCKED + e-mail âmbar; ortogonal a accountFrozen; validado HTTP 10/10 + 2 e-mails Ethereal; SER-39 catalogado (brute-force de 2FA contornável recriando PendingLogin); SER-38 refinado empiricamente (salto de timing pós-lockout); nota técnica TIMESTAMP(3) sem Timestamptz; §1.1 PRE-STAGING mantém 24 itens)
